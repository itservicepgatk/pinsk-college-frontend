...
export function initializeLearners() {
    ...
    DOMElements.importInstructionsBtn.addEventListener('click', () => {
        Swal.fire({
            ...
            html: `<div style="text-align: left; padding: 10px;">
                <p>Для импорта используйте файл <strong>.csv</strong> (разделитель - запятая) с кодировкой <strong>UTF-8</strong>.</p>
                <p>Первая строка - заголовки. Обязательные поля (*):</p>
                <ul>
                    <li><strong>fullName*</strong></li>
                    <li><strong>login*</strong></li>
                    <li><strong>password*</strong></li>
                    <li><strong>group_name*</strong></li>
                    <li><strong>course</strong>, <strong>specialty</strong>, и др.</li>
                </ul></div>`,
    ...
    DOMElements.allLearnersBtn.addEventListener('click', () => {
        updateState({ currentSearchName: '', currentSort: { key: 'full_name', direction: 'asc' }, currentPage: 1, currentGroupName: null });
    ...
    DOMElements.tableHead.addEventListener('click', (e) => {
        ...
        updateState({ currentSort: { key: sortKey, direction }, currentPage: 1 });
        fetchLearners();
    });
    ...
    DOMElements.searchInput.addEventListener('input', () => {
        ...
        searchTimer = setTimeout(() => {
            updateState({ currentSearchName: DOMElements.searchInput.value, currentPage: 1 });
            fetchLearners();
        }, 300);
    });

    DOMElements.groupFilterSelect.addEventListener('change', () => {
        updateState({ currentPage: 1, currentGroupName: DOMElements.groupFilterSelect.value });
        fetchLearners();
    });
